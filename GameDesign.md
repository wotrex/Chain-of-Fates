...
В центрі мапи спавняться табір(намети для жителів), D кількість [жителів](#dweller) та початкові ресурси(зберігаються в спеціальном наметі, але його розмір дуже обмежений). При створенні, [жителі](#dweller) мають випадкові параметри [імені](#dweller-name) та [віку](#dweller-age).

В грі має бути система часу, зміни дня і ночі.
* 1 година = deltaTime тіків.
* 1 день = 24 години.
* 1 сезон = dayPerSeason днів.
* 1 рік = 4 сезона.
...
* <span id="dweller-age">**Вік**</span> - скільки років жителю. Значення за замовчуванням в [параметрах гри](#параметри-гри). Визначається випадково лише при створенні перших жителів на початку гри. В [параметрах гри](#параметри-гри) визначається мін. та макс. значення для генерації віку. Вік підвищується кожен рік у всіх одночасно(Щоб не нагружати гру ще датами народження кожного жителя).Коли вік >= mnA років - впливає на параметри:
    * Максимальна витривалість *= d1%
    * Ефективність роботи -= d2%
    * Швидкість *= d3%
* **Максимальна витривалість** - максимум витривалості який може бути у жилетя. Стандартне значення mxE. На цей параметр впливають параметри вік та голод.
* **Витривалість** - цей параметр зменшується на wE коли житель виконує роботу і збільшується на rE коли житель в стані відпочинку. Це значення не може бути вище значення максимальної витривалості. Стандартне значення = максимальна витривалість. Коли це значення падає нижче mnE значення, то це впливає на параметри:
    * Ефективність роботи -= d4% / витривалість
    * Швидкість -= d5 / витривалість
    * Настрій -= d6 / витривалість
* **Ефективніть роботи %** - це параметр відповідає за швидкість виконання будь-якої роботи. Стандартне значення eW. На цей параметр впливають параметри вік та витривалість.
* **Швидкість** - швидкість руху жителя. Стандартне значення S. На цей параметр впливають параметри вік та витривалість.
* **Рівень достатку** - всього є cLp рівнів достатку. Кожен рівень відповідає за потреби жителя. Чим більше рівень, тим більші потреби. Стандртне значення sLp.
* **Кількість очків до наступного рівня достатку** - значення, яке збільшується на nLp за виконану роботу жителя. Якщо це значення >= потрібної кількості очків, то рівень достатку збільшується, а це значення обнуляється. Коли це значення <= 0, то рівень достатку зменшується, а до цього значення додається значення потрібна кількість очків(після зменшення рівня достатку).
...
* **Кількість очків достатку за d7 кількість днів** - якщо за d7 кількість днів це значення не досягне значення ppdLp, то значення кількості очків до наступного рівня достатку зменшується на lLp.
* **Голод** - кожному жителю потрібна їжа. Стандарне значення H. Кожен день один житель споживає мінімум одну одиницю їжі. Якщо житель спожив їжу, то це значення скидається до стандартного. Якщо житель не спожив їжу, то це значення зменшується на 1. Коли це значення = 0, то житель померає. Коли це значення падає нижче mnH1 значення, то це впливає на параметри:
    * Максимальна витривалість *= d8%
    * Настрій -= d9
    
    Коли це значення падає нижче mnH2 значення, то це впливає на параметри:
    * Максимальна витривалість *= d10%
    * Настрій -= d11
* **Настрій** - значення цього параметру впливає на поведінку жителя. Якщо середній настрій всіх жителів буде >= avarageMood, то в поселення будуть приходить жителі. На цей параметр впливають параметри витривалості та голоду, а також задоволення потреб. Якщо потреба задоволена, то значення цього параметру =+ positiveNecessityValue, якщо потреба не задоволена, то то значення цього параметру =- negativeNecessityValue. Значення цього параметру розраховується сукупністю всіх позитивних і негативних еффектів. Якщо неяких ефектів нема, то це значення = 0.
...
* **Стан** - від цього параметра залежать цілі жителя. Всього є декілька станів:
    ...
    Житель не може перейти з стану роботи в інший стан(окрім стану Небезпека), поки не виконає поточні цілі.
    
    Якщо в стані робота немає цілей, то житель переходить в стан очікування роботи.
#### Поведінка
Ще два параметра жителя це його поточна ціль/задача і наступна ціль/задача. Якщо в нього нема ніяких задач, то за замовчуванням
він повинен випадково блукати по місцевості, але з певними умовами:
* якщо на карті нема будівель, то він випадково блукає;
* якщо на карті є бідівлі, то він випадково блукає, але не може зайти далі, ніж радіус radWandering до найблищої будівлі.
    * якщо на карті є