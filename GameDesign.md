...
5. [Озеро](#lake)
6. [Каміння](#stones)
7. [Поклади заліза](#iron-deposits)
...
Об'єкти на мапі діляться ті з якими можна взаємодіяти і ті з якими не можна взаємодіяти. Ті з якими можна взаємодіяти мають свій запас ресурсів і можуть бути відновлюваними і невідновлюваними. Кожен об'єкт, з яким можна взаємодіяти, дає певний ресурс. Якщо запас ресурса мінімальний і об'єктом уже хтось зайнятий, то він перестає бути видимим для інших [жителів](#dweller). Якщо запас ресурсів об'єкта = 0, то цей об'єкт перестає бути видимим для [жителів](#dweller), а якщо він ще і невідновлювальний, то через `ПЕВНИЙ ЧАС ДЛЯ ВИДАЛЕННЯ` він пропадає.

Об'єкти можуть бути природніми і не природніми, тобто ті що генеруються і ті що з'являються в результаті дій гравця. Не природні об'єкти мають свої властивості.

| Об'єкт                                                             | Природній? | Можна взаємодіяти? | Ресурс       | Запас ресурса         | Відновлювальний | Час на відновлення(ресурс/день) |
|--------------------------------------------------------------------|------------|--------------------|--------------|-----------------------|-----------------|---------------------------------|
| <span id="tree" style="color:green">Дерево</span>                  | Так        | Так                | Деревина     | `ЗАПАС ДЕРЕВА`        | Ні              |                                 |
| <span id="grass" style="color:lime">Трава</span>                   | Так        | Ні                 |              |                       |                 |                                 |
| <span id="berries" style="color:Fuchsia">Ягоди</span>              | Так        | Так                | Ягоди        | `ЗАПАС ЯГІД`          | Так             | `ВІДНОВЛЕННЯ ЯГІД`              |
| <span id="river" style="color:aqua">Річка</span>                   | Так        | Так                | Риба         | `ЗАПАС РИБИ РІЧКА`    | Так             | `ВІДНОВЛЕННЯ РИБИ`              |
| <span id="lake" style="color:aqua">Озеро</span>                    | Так        | Так                | Риба         | `ЗАПАС РИБИ ОЗЕРО`    | Так             | `ВІДНОВЛЕННЯ РИБИ`              |
| <span id="stones" style="color:grey">Каміння</span>                | Так        | Так                | Камінь       | `ЗАПАС КАМІННЯ`       | Ні              |                                 |
| <span id="iron-deposits" style="color:Brown">Поклади заліза</span> | Так        | Так                | Залізна руда | `ЗАПАС ЗАЛІЗНОЇ РУДИ` | Ні              |                                 |
| <span id="building-object">Будівля</span>                                                            | Ні         | Так                |              |                       |                 |                                 |
| <span id="resource-object">Ресурс на землі</span>                                                    | Ні         | Так                |              |                       |                 |                                 |
...
* <span id="wealth-level">**Рівень достатку**</span> - всього є `КІЛЬКІСТЬ РІВНІВ ДОСТАТКУ` рівнів достатку. Кожен рівень відповідає за потреби жителя. Чим більше рівень, тим більші потреби. Стандртне значення `НАЙНИЖЧИЙ РІВЕНЬ ДОСТАТКУ`.
* <span id="wealth-points">**Кількість очків до наступного рівня достатку**</span>  - значення, яке збільшується<!-- на `ПРИРІСТ ДОСТАТКУ ЗА ВИКОНАНУ РОБОТУ`--> за виконану роботу жителя. Якщо це значення >= "потрібної кількості очків до наступного рівня достатку", то "рівень достатку" збільшується, а це значення обнуляється. Коли це значення <= 0, то "рівень достатку" зменшується, а до цього значення додається значення "потрібна кількість очків до наступного рівня достатку"(після зменшення рівня достатку). Рівень достатку не може зменшитися, коли досягая найнижчиго рівня.
...
* <span id="hunger">**Голод**</span> - кожному жителю потрібна їжа. Стандарне значення `БАЗОВИЙ ГОЛОД`. Кожен день один житель споживає мінімум одну одиницю їжу. Якщо житель спожив їжу, то це значення зменшується на 1. Якщо житель не спожив їжу, то це значення збільшується на 1. Коли це значення = 'МАКСИМАЛЬНИЙ РІВЕНЬ ГОЛОДУ', то житель померає. Коли це значення падає нижче `1 РІВЕНЬ ГОЛОДУ` значення, то це впливає на параметри:
...
Інші дії представляють собою виконання чогось на одному місті. В кожной із них є свій час виконання та затрати [витривалості](#dweller-endurance)(витривалість забирається на початку виконання дії, а не в кінці). На час виконання дії впливає [ефективність роботи](#work-efficiency)(Час виконання *= ефективність роботи). 

 Деякі дії використовують або створюють ресурс/предмет в [інвентарі для доставки](#delivery-inventory) жителя. Створюваний ресурс переноситься в інвентар, ресурс який потрібен також повинен бути в інветарі і видаляється коли дія завершується. 
 
 Деякі дії взаємодіють з об'єктами на мапі(дерева, каміння поклади заліза і т.д.) та змінюють їхні параметри, об'єкт обов'язково повинен знаходитися поруч.
 
 Кожна дія відрізняється анімацією. Дія завершається тоді, коли завершиться таймер виконання цієї дії.
...
<span id="take-the-object">**Взяти предмет | TO (*Take the Object*)**</span> - ця дія дозволяє взяти ресурс/предмет з інвентаря будівлі або [землі](#resource-object). Ресурс/предмет або будівля з ним повинні знаходитися близько до жителя в радіусі `РАДІУС ДЛЯ ПІДНЯТТЯ ПРЕДМЕТА`, щоб житель міг взяти його, інашке дія завершується. Якщо в жителя в інвентарі вже є інша річ, то вона викидається. Якщо в жителя в інвентарі вже є річ яку він бере, то береться певна кількість, яка не перевищує максимальну вагу(вага предмета * кількість <= `МАКСИМАЛЬНА ПЕРЕНОСИМА ВАГА`).  Ця дія завжди приймає значення ЩО за ресурс/предмет, який треба підібрати. Якщо не вказано який ресурс треба взяти, то береться перший або випадковий ресурс близько.

<span id="to-put-an-object">**Покласти предмет | PO (*to Put the Object*)**</span> - кладе в будівлю або на землю ресурс/предмет, який знаходиться в [інвентарі](#delivery-inventory). Якщо житель знаходиться в будівлі, то ресурс/предмет переміщується в інвентар будівлі. Якщо житель знаходиться не в будівлі, то ресурс/предмет переміщується на землю, тобто створюється [об'єкт](#resource-object) на мапі з параметрами цього ресурса, а ресурс в інвентарі жителя видаляється.
...
Житель завжди має якусь **задачу**. Кожна задача описує порядок дій та їхні цілі. Кожна задача належить якомусь стану. Деякі задачі мають умови виконання. Якщо задача скасувалася, то поточні дії також скасовуються. Є відмінювані задачі і невідмінні, вони відрізняються тим, що невідмінні не скасовуються коли житель переходить в інший стан, окрім стану [небезпека](#danger). 

Якщо дія по якісь причині відмінилися, то вона виконується знову, але якщо в переліку дій перед нею стояла дія **W**, то виконується ця дія **W**.

За повне виконання деяких задач(в основном професійних) росте [достаток жителя](#wealth-points). За виконання деяких задач може бути ще якись еффект.

| Задача     | Відмінювана | Порядок дій    | Умови | Стан       | Професія | +/-Достаток | Додатково |
|------------|-------------|----------------|-------|------------|----------|-----------|-----------|
| Прогулянка | Так         | **W**(1) **R** |       | Відпочинок |          |           |           |
|            |             |                |       |            |          |           |           |

<details>
<summary>Цілі для дій в кожній задачі(Циферка після дії. Прик.: W(1))</summary>

**Прогулянка**

1. Випадкова точка на мапі. Не за межами стін. Якщо стін нема, то в радіусі `РАДІУС ДЛЯ ПРОГУЛЯНКИ` від ближчої будівлі.
</details>

**Стан** - від цього параметра залежать задачі жителя. Всього є декілька станів:
1. Відпочинок - стандартний стан. В цьому стані житель не виконує роботу і трохи відновлює витривалість. Задачі цього стану обираються випадково(або по порядку можна зробити).
2. Робота - стан коли житель виконує задачі, які визначаються його професією.
3. Очікування роботи - стан який завжди має ідентичний розклад з станом робота. Коли в стані робота нема задач, то житель переходить в цей стан. Загалом цей стан має ті самі задачі що і стан відпочинок. Коли в його професії з'являються задачі він відразу переходить в стан Робота.
4. Сон - стан, в якому житель йде додому і спить. Цей стан має обмеження - його кількість годин в день його розклада не може бути більше `ЧАС ДЛЯ СТАНУ СОН`.
5. <span id="danger">Небезпека</span> - унікальний стан, який не має розклад. Житель переходить в цей стан у разі якойсь загрози(напад ворогів) терміново, тобто житель відразу відміняє свої задачі і виконує задачі цього стану. Під час цього стану житель ховається. 

Безробітні не мають станів "робота" та "очікування роботи". Для кожного стану є розклад(окрім стану "Небезпека") - в якусь годину житель переходе з одного стану в інший. Розклад в [параметрах](#параметри-гри). Розклад для кожної професії може змінюватися в будівлі цієї професії, тому як тільки житель отримав професію - його розклад визначається будівлею, де він працює.
...
#### Потреби
Потреби бувають двох видів - життєво важливі і додаткові. За не виконання життєво важливих потреб у жителя падає [настрій](#mood). Додаткові потреби за не виконання не дають штрафи до настрою, але за виконання дають бонус до настрою. Кожний [рівень достатку](#wealth-level) має свої потреби. На вищому рівню достатку потреба може зникнути, може з'явитися нова, або додаткова потреба може стати життєво вижливою.

Кожна потреба має свій час до наступної перевірки потреби. Коли час випливає, то перевіряється чи виконана потреба. Якщо це ресурс, то він споживається(зі складу) відразу перед перевіркою, якщо спожити не було що, то потреба не виконана. Якщо потреба це задача, то перевіряється чи була виконана ця задача до цього. Якщо задача була виконана то потреба виконується.

Параметр жителя [голод](#hunger) пов'язаний з потребами. Якщо житель споживає будь-який ресурс їжі, то його голод зменшується на 1, якщо всі потреби з їжею не виконані, то голод збільшується на 1.

Деякі потреби при виконанні забирають [достаток](#wealth-points).
